...
/*void calculateTFactor(double* a1 , double* b1 ,  double* a2 , double* b2 ,  int size , double* s1 , double* s2){
	#pragma omp for  schedule(runtime)  reduction(+:*s1,*s2)
	for(int i = 0; i < size; ++i){
		s1+=(a1[i] * b1[i]);
		s2+=(a2[i] * b2[i]);
	}
...
int main(int argc , char** argv){
	...
	#pragma omp parallel
	{
		...
		#pragma omp for  schedule(runtime)  reduction(+:absolute_b)
		...
		while(1){
			...
			#pragma omp for  schedule(runtime)  reduction(+:absolute_vec)
			for(int i = 0; i < MATRIX_SIZE; ++i){
			...
			#pragma omp for  schedule(runtime)  reduction(+:s1)
			for(int i = 0; i < MATRIX_SIZE; ++i){
				s1+=(vec_y[i] * vec_keep[i]);
			}

			#pragma omp for  schedule(runtime)  reduction(+:s2)
			for(int i = 0; i < MATRIX_SIZE; ++i){
				s2+=(vec_keep[i] * vec_keep[i]);
			}
